 ...
 * real, distinct query (verified via web search, 2026-08-25: multiple
 * independent chess-education sites publish dedicated pages on repertoire
 * BREADTH specifically) that this site's existing guides touch only in
 * passing: how-to-build-your-opening-repertoire.js's Step 5 has one
 * sentence on it ("Four openings, understood, beat twelve memorized," now
 * linking here) inside a step about post-selection maintenance, not a
 * dedicated treatment; upgrade-your-repertoire-as-you-improve.js covers
 * WHEN to deepen an existing opening as rating rises (not the starting
 * breadth question); should-you-study-openings-under-1500.js covers
 * whether opening study is worth the TIME at all (not how many openings to
 * split that time across). None of the three answers "how many" as its own
 * question -- same "one broad guide touches it lightly, a dedicated guide
 * goes deep" pattern already established by
 * opening-strategy-by-time-control.js vs. rapid-chess-opening-prep.js (see
 ...
 * not a score, just an honest number used as a worked example of how narrow
 * a real repertoire can be.
 */
...
const meta = {
  ...
  related: ['how-to-build-your-opening-repertoire', 'should-you-study-openings-under-1500'],
};
...
function render(ctx) {
  ...
  return `
    ...
    <p>This build tracks ${entries.length} openings total - ${whiteCount} for White, ${blackCount} for Black - deliberately not an exhaustive list, closer in size to what the advice above recommends a single player actually carry. You don&rsquo;t need all of them: picking one per color you actually enjoy playing, and going deep on those, is a complete repertoire by the standard above. See <a href="openings.html">the full comparison &rarr;</a> to browse what this site tracks, or a rating-banded ranking (<a href="best-white-openings-1600-1800.html">best White openings at 1600-1800 &rarr;</a>, <a href="best-black-openings-1600-1800.html">best Black openings at 1600-1800 &rarr;</a>, and the other bands linked from each) if you want a starting point narrowed by how it actually scores at your level.</p>
