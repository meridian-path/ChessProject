...
function createPage(band) {
  ...
  function render(ctx) {
    ...
    const items = all
      ...
      .map((m) => {
        const follow = m.punishingReply
          ...
          : '';
        return `<li class="callout">In the <a href="${escapeHtml(m.slug)}.html">${escapeHtml(m.name)}</a>, <strong>${escapeHtml(m.san)}</strong> is played in ${formatPct(m.playedPct)}% of games at ${escapeHtml(m.band)} but scores only ${formatPct(m.score)}% for ${escapeHtml(m.opponentColor)}.${follow}</li>`;
      })
      .join('');
    ...
    return `
      ...
      <h2>Go deeper</h2>
      <p>Each opening's own page has the full breakdown by rating band, top replies, and real recent club games. See the <a href="openings.html">full openings comparison &rarr;</a>, or see how these same mistakes look at other rating bands: ${bandNavLinks(band, escapeHtml)}.</p>
    `;
  }
