...
function render(ctx) {
  ...
  const mistakesHtml = hasMistakes
    ...
    <p><strong>${escapeHtml(topScoring.san)}</strong> is Black&rsquo;s highest-scoring reply on this exact table, at ${formatPct(topScoring.blackScore)}% from ${topScoring.games.toLocaleString()} games (played ${formatPct(topScoring.playedPct)}% of the time). That gap between popularity and result is the real, checkable answer this data can offer to &ldquo;how to beat the London.&rdquo;</p>`
